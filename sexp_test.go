...
func TestLen(t *testing.T) {
	...
	sxp = Cons(Cons(0, 1), 2)
	if sxp.Len() != 2 { t.Fatalf("With 1 nested Cons cells the length should be 2 but is %v", sxp.Len()) }

	sxp = Cons(0, 1, Cons(2, Cons(3, 4, 5)), Cons(6, 7, 8, 9))
	if sxp.Len() != 4 { t.Fatalf("With 2 nested Cons cells the length should be 3 but is %v", sxp.Len()) }

	sxp = Cons(0, 1, Cons(2, Cons(3, 4, 5)), sxp, Cons(6, 7, 8, 9))
	if sxp.Len() != 5 { t.Fatalf("With 2 nested Cons cells plus recursion the length should be 5 but is %v", sxp.Len()) }
}
